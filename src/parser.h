...
enum Type { GATE, NET, INPUT, OUTPUT };

enum GateType { AND, OR, NAND, NOR, NOT, BUF, XOR, XNOR };
...
public:
  Based(const string &name, Type type, int id = 0)
...
protected:
  ...
  vector<Based *> gates;
  map<string, Based *>
      inputsMap; // to quick find pointer(Based*) from name(string)
  map<string, Based *>
      outputsMap; // to quick find pointer(Based*) from name(string)
