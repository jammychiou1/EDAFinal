...
void Mat::add_row_or_col_i_to_j(int i, int j, u128 c, bool transpose) {
  for (int k = 0; k < get_h_or_w(!transpose); k++) {
    at(j, k, transpose) =
        (at(j, k, transpose) + c * at(i, k, transpose)) & mask;
  }
}
...
Mat::Mat(vector<unsigned __int128> v, u128 mask)
    : Mat(pair<int, int>{v.size(), 1}, mask) {
  ...
  for (int i = 0; i < get_h_or_w(false); i++) {
    for (int j = 0; j < A.get_h_or_w(true); j++) {
      for (int k = 0; k < get_h_or_w(true); k++) {
        result.data[i][j] =
            (result.data[i][j] + data[i][k] * A.data[k][j]) & mask;
      }
    }
  }
  return result;
  ...
  for (int i2 = i + 1; i2 < A.get_h_or_w(transpose); i2++) {
    ...
    if (A.at(i2, i, transpose) != 0) {
      ...
      while (A.at(i, i, transpose) != 0) {
        // cout << "i, i and i2, i " << u128_to_string(A.at(i, i, transpose)) <<
        // ' ' << u128_to_string(A.at(i2, i, transpose)) << '\n';
    ...
    // cout << (unsigned long long) A.at(i, i, transpose) << ' ' << (unsigned
    // long long) A.at(i2, i, transpose) << '\n'; cout << "A\n"; A.print(); cout
    // << endl; char d; cin >> d;
  }
  return flag;
}
...
u128 calc_term(const vector<u128> &input_sample, const termdec &term,
               u128 mask) {
  u128 res = 1;
...
vector<u128> gen_row(const vector<u128> &input_sample,
                     const vector<termdec> &terms, u128 mask) {
  vector<u128> res;
...
void n_m_simple_terms(int n, int m, int num_inputs, vector<termdec> &result,
                      termdec &term_now, int prefix_sum = 0) {
...
pair<bool, map<termdec, i128>> solve_single(map<string, vector<u128>> inputs,
                                            vector<u128> output, int width,
                                            int n, int m) {
...
pair<bool, vector<map<termdec, i128>>>
solve(map<string, vector<bitvec>> inputs, map<string, vector<bitvec>> outputs) {
  if (inputs.size() > 6) {
    // cout << "here\n";
  ...
  for (auto &p : outputs) {
    ...
    pair<bool, map<termdec, i128>> res =
        solve_single(inputs_u128, output_u128, width, 1, 5);
    if (res.first) {
