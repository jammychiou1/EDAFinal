...
void Converter::convert(string path,
                        vector<map<vector<int>, __int128>> result) {
  ofstream file(path);
  // for (auto mp: result) {
  //     for (auto term: mp) {
...
void Converter::convert_control(string path,
                                vector<map<vector<int>, __int128>> result,
                                string control_input, int val) {
  ofstream file;
  if (!pre) {
    pre = true;
    file.open(path);
...
string
Converter::generate_lines(std::vector<std::pair<std::string, int>> &group,
                          std::string type) {
  string paragraph = "";
  for (auto &item : group) {
    ...
    if (item.second == -1) {
      ...
      line =
          line + "[" + to_string(item.second - 1) + ":0] " + item.first + ";\n";
    }
    paragraph += line;
  }
  return paragraph;
...
void Converter::prewrite(ofstream &file) {
  string module_start = "module top(";
  for (auto &in : inputs)
    module_start += in.first + ", ";
  for (auto &out : outputs)
    module_start += out.first + ", ";
...
void Converter::inwrite(ofstream &file,
                        vector<map<vector<int>, __int128>> result,
                        string control_input, int val) {
...
void Converter::postwrite(ofstream &file) {
