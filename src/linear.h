...
public:
  ...
  int get_h_or_w(bool transpose) const {
    return transpose ? data[0].size() : data.size();
  }
...
std::pair<bool, Mat>
solve_mod_pow_2(std::vector<std::vector<unsigned __int128>> AA,
                std::vector<unsigned __int128> bb, int pow);
std::pair<bool, std::map<std::vector<int>, __int128>>
solve_single(std::map<std::string, std::vector<unsigned __int128>> inputs,
             std::vector<unsigned __int128> output, int width, int n, int m);
...
//     }, // for out2
// ]
std::pair<bool, std::vector<std::map<std::vector<int>, __int128>>>
solve(std::map<std::string, std::vector<std::vector<bool>>> inputs,
      std::map<std::string, std::vector<std::vector<bool>>> outputs);
